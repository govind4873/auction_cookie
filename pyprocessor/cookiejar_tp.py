...
class CookieJarTransactionHandler(TransactionHandler):
	...
	name = payload_list[2]
        ...
        LOGGER.info("bid Amount = %s.", amount)
	LOGGER.info("supplier = %s.", name)
        if action == "bid":
            self._make_bid(context, amount, name, from_key)
	
        else:
            LOGGER.info("Unhandled action. Action should be bid")
