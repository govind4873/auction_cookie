...
# ------------------------------------------------------------------------------
'''
Auction in supply chain class interfaces with Sawtooth through the REST API.
...
class CookieJarClient(object):
    ...
    Supports "bid", and "count" functions.
    '''
    ...
    # 2. Send to REST API
    def bake(self, amount, name):
        '''send the bid details to the validator.'''
        return self._wrap_and_send("bid", amount, name, wait=10)

    def count(self):
        '''return the winner of the bid .'''
    ...
    def _wrap_and_send(self, action, amount, name, wait=None):
        ...
        raw_payload = ",".join([action, str(amount), name])
